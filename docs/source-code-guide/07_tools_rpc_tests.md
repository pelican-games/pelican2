...
[`src/devcli/main.cpp`](../../src/devcli/main.cpp#L9) は第1引数を見て4系統へそのまま dispatch します。

```text
pelican_cli
├─ assets <manifest|verify|status>
├─ import <delivery_dir> --project <dir|project.json>
...
[`runProjectInitCommand()`](../../src/devcli/projectinit.cpp#L380) は、空であることを確認した directory に template file 群を書きます。template の一覧は [`templateFiles()`](../../src/devcli/projectinit.cpp#L359) にあります。
...
| `passes/...json` | target、pass、frame graph の初期設定 |
| `code/CMakeLists.txt`, game `.cpp` | game system を player へ組み込む project code |
...
[`jsonrpc.hpp`](../../src/project/jsonrpc.hpp#L12) と [`jsonrpc.cpp`](../../src/project/jsonrpc.cpp#L121) は engine module を知りません。
...
[`RpcServer`](../../src/core/communication/rpcserver.hpp#L18) は `istream` / `ostream` と method handler map を持ちます。1行を1 request とし、[`handleLine()`](../../src/core/communication/rpcserver.cpp#L484) で parse → handler → response serialize を行います。[`run()`](../../src/core/communication/rpcserver.cpp#L510) は EOF まで1行ずつ読み、必ず1行の response を flush します。

transport が socket ではなく stream interface なのがポイントです。production 起動では stdin/stdout、unit test では stringstream を差し替えられます。

engine method の登録は [`runEngineRpcServer()`](../../src/core/communication/rpcserver.cpp#L521) に集約されています。

| method | 実装行 | 状態変更 |
|---|---|---|
| `get_status` | [`rpcserver.cpp`](../../src/core/communication/rpcserver.cpp#L526) | instance、project、scene、frame/time、seed、store 状態を返す |
| `set_seed` | [`rpcserver.cpp`](../../src/core/communication/rpcserver.cpp#L540) | deterministic RNG を reseed |
| `inject_input` | [`rpcserver.cpp`](../../src/core/communication/rpcserver.cpp#L548) | canonical input queue へ key/mouse/axis event を積む |
| `inject_event` | [`rpcserver.cpp`](../../src/core/communication/rpcserver.cpp#L557) | 名前から登録済み event layer へ JSON payload を積む |
| `set_time` | [`rpcserver.cpp`](../../src/core/communication/rpcserver.cpp#L571) | time を直接設定。frame index は進めない |
| `update_transforms` | [`rpcserver.cpp`](../../src/core/communication/rpcserver.cpp#L577) | update を pending queue へ積む |
| `load_gltf` | [`rpcserver.cpp`](../../src/core/communication/rpcserver.cpp#L586) | transient glTF を scene に追加 |
| `load_scene` | [`rpcserver.cpp`](../../src/core/communication/rpcserver.cpp#L597) | scene を clear/load、pending transforms を破棄 |
| `set_camera` | [`rpcserver.cpp`](../../src/core/communication/rpcserver.cpp#L606) | 名前付き object を active camera にする |
| `step_frame` | [`rpcserver.cpp`](../../src/core/communication/rpcserver.cpp#L614) | pending flush → time advance → 5 phase update → render |
| `render_frame` | [`rpcserver.cpp`](../../src/core/communication/rpcserver.cpp#L624) | time/frame を進めず、pending flush → seq update → render |
| `get_frame_plan` | [`rpcserver.cpp`](../../src/core/communication/rpcserver.cpp#L632) | planner の JSON を返す |
| `capture` | [`rpcserver.cpp`](../../src/core/communication/rpcserver.cpp#L637) | headless の最後の frame を PNG 保存 |
...
transform update も即適用ではなく pending です。複数 update をまとめてから `step_frame` / `render_frame` の境界で [`flushPendingTransforms()`](../../src/core/communication/rpcserver.cpp#L429) します。

### protocol 上の注意

- stdout は JSON-RPC 専用です。通常 log を stdout へ混ぜると client の1行 protocol を壊します。
- `capture` は `OffscreenFrameTarget` の readback が前提で、window swapchain では未対応です。
- `inject_event` は名前で登録された event type にだけ届きます。payload は登録型の binder が解釈します。
- method handler の通常例外は application error `-32000` に正規化されます。[`handleLine()` の catch](../../src/core/communication/rpcserver.cpp#L484) を参照してください。
...
[`golden_image_test.cpp`](../../test/golden_image_test.cpp#L1526) は headless render 結果を [`test/golden/`](../../test/golden) の `expected.png` と比較します。triangle、fullscreen、HDR on/off、shadow、debug draw/text、compute buffer、camera、VAT などが独立 case です。

この test は画像一致だけでなく、[`Renderer execution matches plan order`](../../test/golden_image_test.cpp#L1566) で planner の node 順と実行 trace が同じこと、[`fullscreen inputs rebind...`](../../test/golden_image_test.cpp#L1604) で hot reload/resize 後の descriptor 再結合も確認します。描画 refactor 時の最も強い回帰防止線です。

...
| Studio project editor | Widgets shell、dock、versioned layout preset、エンジン非起動の project load と read-only outliner、別 process player の native viewport 埋め込みまで実装済み(D1 完了)。viewport と outliner の project 選択連携、scene editing、engine RPC は未接続。**ただしエンジン側の編集面(編集 RPC / ImGui inspector)は実装済み**なので、対比して読むこと | [`MainWindow`](../../src/devstudio/view/mainwindow.cpp#L69) / [`EmbeddedViewport`](../../src/devstudio/viewport/embeddedviewport.hpp#L16) |

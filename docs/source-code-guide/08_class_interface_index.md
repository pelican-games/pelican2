...
| `FullscreenPassWidget` | draft-only Qt form | [`fullscreenpasswidget.hpp`](../../src/devstudio/view/fullscreenpasswidget.hpp#L22) | [`fullscreenpasswidget.cpp`](../../src/devstudio/view/fullscreenpasswidget.cpp) | frame-plan resource 候補と read-only 著作宣言から `fullscreen` の部分 JSON、omitted keys、三値の検査軸を表示。project write と engine apply は持たない |

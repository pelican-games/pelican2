...
| `RuntimeTeardownStep` / `runtime_teardown_order` / `RuntimeTeardownActions` | 規範順序の値 | [`teardown.hpp` 内](../../src/core/appflow/teardown.hpp#L11) / [`#L22`](../../src/core/appflow/teardown.hpp#L22) / [`#L49`](../../src/core/appflow/teardown.hpp#L49) | 同左 | 8 段階の teardown 順序を型で固定。空 action は「起動前に落ちた module」 |
| `RuntimeTeardownMode` | enum | [`teardown.hpp` 内](../../src/core/appflow/teardown.hpp#L60) | 同左 | `runtime_reset` / `terminal_shutdown` |
...
| `AuthoringSceneDocumentStage` | 未公開の構造編集 | [`AuthoringSceneDocumentStage`](../../src/core/loader/authoringscenedocument.hpp#L130) | 同左 | insert / remove / restore / rename / reorder のみ。`finish(revision) &&` で確定 |
| `AuthoringObjectClosure` | 復元用クロージャ | [`AuthoringObjectClosure`](../../src/core/loader/authoringscenedocument.hpp#L54) | 同左 | `removeObject` の可逆情報(同じ `AuthoringObjectId` を同じ宣言区間へ戻す) |
...
| `SceneSaveErrorCode` / `SceneSaveError` / `SceneSaveResult` | 保存契約 | [`SceneSaveErrorCode`](../../src/core/loader/basicconfig.hpp#L19) / [`SceneSaveError`](../../src/core/loader/basicconfig.hpp#L25) / [`SceneSaveResult`](../../src/core/loader/basicconfig.hpp#L48) | [`saveSceneDocument()`](../../src/core/loader/basicconfig.cpp#L693) | `ExternalModification` / `Unavailable` / `IoFailure` の型付き保存エラー |
...
| `decodeVrmaAnimation()` / `loadVrmaAnimation()` | free function | [`vrmadecoder.hpp` 内](../../src/core/loader/vrmadecoder.hpp#L25) / [`#L32`](../../src/core/loader/vrmadecoder.hpp#L32) | [`vrmadecoder.cpp`](../../src/core/loader/vrmadecoder.cpp) | `.vrma` GLB decode。拡張子ゲートと content hash |
| `VrmaRetargetProfile` / `retargetVrmaClip()`(retarget = ある骨格向けに作られたアニメーションを、骨構成や rest 姿勢が違う別の骨格で再生できるよう変換すること) | 版付き profile | [`VrmaRetargetProfile`](../../src/core/animation/vrmaretarget.hpp#L67) / [`#L124`](../../src/core/animation/vrmaretarget.hpp#L124) | [`vrmaretarget.cpp`](../../src/core/animation/vrmaretarget.cpp) | target-rig-local な不変中間クリップ。**`AnimationSource` ではない** |
...
| `eventPayloadPolicy` / `behaviorParamsPolicy` / `componentPolicy()` | policy | [同](../../src/core/userpublic/details/schema/structfieldschema.hpp#L83) / [#L84](../../src/core/userpublic/details/schema/structfieldschema.hpp#L84) / [`componentPolicy()`](../../src/core/userpublic/details/schema/structfieldschema.hpp#L86) | 同左 | 3 種の use-site 制約 |
...
| `PreparedComponentValue<T>` / `PreparedComponentSwap<T>` | 値の prepare/publish | [`coretemplate.hpp` 内](../../src/core/userpublic/details/ecs/coretemplate.hpp#L217) / [`#L228`](../../src/core/userpublic/details/ecs/coretemplate.hpp#L228) | 同 header | 投げうるコピーを公開前に済ませ、publish/rollback は no-throw 代入 + version 交換だけ |
...
| 名前 | 形 | 宣言 | 主実装 | 責務 |
|---|---|---|---|---|
| `Renderer` | module/orchestrator | [`renderer.hpp`](../../src/core/vkcore/renderer.hpp#L47) | [`renderLogicalFrame()`](../../src/core/vkcore/renderer.cpp#L3676) / [`render()`](../../src/core/vkcore/renderer.cpp#L4481) | hot reload、graph variant、logical frame(multi-view)、trace を束ねる |
...
| `PreviewGraphProgram` / `precompilePreviewGraph()` | data-only graph | [`CompiledRenderPipeline`](../../src/core/renderingpass/previewgraph.hpp#L15) / [`#L27`](../../src/core/renderingpass/previewgraph.hpp#L27) | [`previewgraph.cpp`](../../src/core/renderingpass/previewgraph.cpp) | 第3の graph variant。`RenderingPassId` を持たず `renderLogicalFrame` を通らない |
...
| `FrameGraphDefinition` | pure graph | [`frameplanner.hpp`](../../src/core/renderingpass/frameplanner.hpp#L32) | [`parseFrameGraphDefinitionsFromConfigJson()`](../../src/core/renderingpass/frameplanner.cpp#L1608) | node declarations と既知 resource |
...
| `FrameGraphRuntimeContainer` | module/binder | [`framegraphruntime.hpp`](../../src/core/renderingpass/framegraphruntime.hpp#L32) | [`registerExecutionPlan()`](../../src/core/renderingpass/framegraphruntime.cpp#L577) | plan の node 名を compiled pass/task index へ bind |
...
| `FrameGraphResourceContainer` | module/GPU owner | [`computetask.hpp`](../../src/core/renderingpass/computetask.hpp#L40) | [`registerBuffers()`](../../src/core/renderingpass/computetask.cpp#L1631) | named storage buffers と descriptor info |
| `ComputeTaskContainer` | module/runtime registry | [`computetask.hpp`](../../src/core/renderingpass/computetask.hpp#L59) | [`registerComputeTask()`](../../src/core/renderingpass/computetask.cpp#L2176) | compute pipeline、descriptor、dispatch group、barrier |
| `RenderPassExecutor` | module/executor | [`render_pass_executor.hpp`](../../src/core/vkcore/render_pass_executor.hpp#L20) | [`execute()`](../../src/core/vkcore/render_pass_executor.cpp#L314) | layout transition、Dynamic Rendering scope、variant dispatch |
...
| `BufferWrapper` / `ImageWrapper` | RAII value | [`buf.hpp`](../../src/core/vkcore/buf.hpp#L7) / [`image.hpp`](../../src/core/vkcore/image.hpp#L7) | [`VulkanManageCore::allocBuf()`](../../src/core/vkcore/core.cpp#L930) | Vulkan object と VMA allocation を同居させる |
...
| `DeletionQueueCore` | type-erased deferred owner | [`deletionqueue.hpp`](../../src/core/vkcore/deletionqueue.hpp#L16) | [`beginFrame()`](../../src/core/vkcore/deletionqueue.cpp#L56) | frames-in-flight 後まで旧 GPU object の destructor を遅延。teardown 後は [`requireAccepting()`](../../src/core/vkcore/deletionqueue.hpp#L80) が `defer()` を拒否 |
| `ShaderCompiler` | module/service | [`shadercompiler.hpp`](../../src/core/shader/shadercompiler.hpp#L27) | [`compileFile()`](../../src/core/shader/shadercompiler.cpp#L578) | GLSL source + define/include を SPIR-V へ compile |
...
| `PipelineFactory` | module/factory+registry | [`pipelinefactory.hpp`](../../src/core/shader/pipelinefactory.hpp#L59) | [`buildGraphicsPipeline()`](../../src/core/shader/pipelinefactory.cpp#L491) | reflection-driven layout、graphics/compute pipeline、cache、hot rebuild |
...
| `MaterialContainer` | module/GPU owner | [`materialcontainer.hpp`](../../src/core/material/materialcontainer.hpp#L34) | [`registerMaterial()`](../../src/core/material/materialcontainer.cpp#L1730) | texture、material descriptor/pipeline、push constants |
...
| `GpuTimingSubrange` / `GpuTimingSampleIdentity` / `makeGpuTimingSampleLabel()` | GPU timing | [`GpuTimingSubrange`](../../src/core/vkcore/rendertiming.hpp#L25) / [`#L42`](../../src/core/vkcore/rendertiming.hpp#L42) / [`#L75`](../../src/core/vkcore/rendertiming.hpp#L75) | [`rendertiming.cpp`](../../src/core/vkcore/rendertiming.cpp) | view 分離 + `barriers`/`body` 分割。ラベルは debug utils と同一命名 |
...
| `LightContainer` | module/GPU owner | [`lightcontainer.hpp`](../../src/core/light/lightcontainer.hpp#L21) | [`load()`](../../src/core/light/lightcontainer.cpp#L253) | directional/point/spot light、shadow VP、GPU light buffer |
...
| `FullscreenPassContainer` | module/pass registry | [`fullscreenpasscontainer.hpp`](../../src/core/fullscreenpass/fullscreenpasscontainer.hpp#L16) | [`registerFullscreenPass()`](../../src/core/fullscreenpass/fullscreenpasscontainer.cpp#L184) | fullscreen pipeline と input descriptors |
...
| `EditorCommandRpcAdapter` / `EditorCommandImGuiFakeAdapter` | adapter | [`EditorCommandRpcAdapter`](../../src/core/communication/editorcommandservice.hpp#L290) / [`EditorCommandImGuiFakeAdapter`](../../src/core/communication/editorcommandservice.hpp#L328) | 同左 | RPC と ImGui が **同じサービス**を呼ぶことの担保 |

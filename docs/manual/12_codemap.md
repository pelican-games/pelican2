...
1. **CLI パース** — [`parseLaunchConfig()`](../../src/player/main.cpp#L227) `parseLaunchConfig`。argparse で全引数を解釈し、[EngineLaunchConfig](../../src/core/launchconfig.hpp) に詰める。`--project` 省略時の暗黙探索は [`findExampleProjectNearExecutable()`](../../src/player/main.cpp#L166) `findExampleProjectNearExecutable`(exe の祖先から `projects/example/project.json` を探す)
2. **コア生成** — [`main.cpp` 内](../../src/player/main.cpp#L438) `main` → `Pelican::PelicanCore pl{...}`([pelican_core.cpp](../../src/core/userpublic/pelican_core.cpp))。モジュールコンテナに LaunchConfig を書き込み、解析済み`ProjectEnvelope`を[PathResolver](../../src/core/loader/pathresolver.hpp)`.setup()`へ、raw JSONを[ProjectSource](../../src/core/loader/projectsrc.hpp)へ渡す
...
| やりたいこと | 読む / 変える場所 | 参照章 |
|---|---|---|
| CLI 引数を足す | [`parseLaunchConfig()`](../../src/player/main.cpp#L227) + [launchconfig.hpp](../../src/core/launchconfig.hpp) | [第10章](10_tools.md) |

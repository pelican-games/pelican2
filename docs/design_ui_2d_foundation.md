...
### 3-1. 語彙の拡張 3 段(2026-07-10 ユーザーレビュー反映 — enum を育てない)

| 段 | 手段 | 例 |
|----|------|-----|
| 合成 | 既存 6 種 + emit の組み合わせ(コードなし) | 確認ダイアログ、リスト風の並び |
| **`PELICAN_REGISTER_WIDGET`** | プロジェクト C++ がウィジェット型を登録(黒魔術登録の系譜)。Controller がポインタイベントを受けてサブツリーと値を更新。JSON の未知 type が登録型として解決される | ドラッグ数値入力、カスタムミニマップ |
| ツールウィジェットパック | drag_number / checkbox / slider / color_picker を engine:// コンテンツとして同梱 — **ただし上記②の公開機構だけで実装**(標準に特権なし。エンジン自身のデバッグパネルが dogfood) | ゲーム内デバッグ/チューニング UI |

- 登録型にも未定義の type は名前入りエラー(web は skip + WARN —
  サブセット原則の scene と同じ扱い)
- **基盤への波及要件(この検討で確定)**: ①バッチャ v1 の頂点フォーマットに
  **per-vertex color** を含める(カラーピッカーの HSV 面 = グラデーション
  クアッド。後から頂点レイアウトを変えると全シェーダ再コンパイルになるため
  最初から)②イベント語彙に **capture 付き drag**(`on_drag(delta)`、
  press〜release 間のポインタ占有)を v1 で含める

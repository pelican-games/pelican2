...
- 構文: `<パス>#<種別>/<名前>`。種別はコンテナごとに定義
  (glb: mesh / material / node / animation。アトラス: sprite。追加は
  エンジン先行 — サブセット原則)
- 名前はコンテナ内で一意であること。重複は**ロード時エラー**(名前 + パスを
  出す)。インデックス参照(`#mesh/3`)は不採用(DCC の並び替えで壊れる)
...
- **設定の 4 層**(上が優先): ローカル(.pelican/local.json)>
  プロジェクト(imports.rules.json)> プロジェクトの既定差し替え
  (同ファイルの `defaults` 区画で拡張子既定レシピを上書き/無効化)>
  エンジン組み込み既定。**既定値も含めて全部データ** — 大規模開発で
  外部アセット管理ツールが全層を自前生成してよい
...
- 生成物を直接編集した状態での再 import は**差分検出して警告**(manifest の
  sha256 で機械判定できる)

...
- 構文: `<パス>#<種別>/<名前または連結パス>`。種別はコンテナごとに定義
  ...
  エンジン先行 — サブセット原則)
- **連結パス対応(2026-07-08)**: 階層を持つコンテナは親からの連結で指せる
  (`#node/Root/Arm/Cube`)— DCC 出力の同名要素(Cube.001 問題)をある程度
  区別できる。一意性の要求は**同一フルパス単位**。それでも重複する glb は
  **開発ルール違反**(コンテナ内の同名回避はチームの規律 — refs check が指摘、
  エンジンはロード時エラーで名前 + パスを出す)。

...
### 0-1. ユーザー管理枠とエンジン語彙の境界表(規範 — 2026-07-16 確定)

判定基準 = **「5 年後に発展しているのはどちら側か」**。品質・
アルゴリズム(毎年手法が出る側)は全部ユーザー枠、データ供給と
安全境界(発展しない不変条件)だけエンジン。

**ユーザー管理(コピーして改造 = 自分のもの)**:

| 部品 | 自由度 |
|------|--------|
| taa.json(パス構成・RT 宣言・アンカー・reads/writes)| パス段数・履歴面数とも自由 |
| resolve/composite シェーダ | クランプ方式・disocclusion・ブレンド・responsive マスク等、品質の全部 |
| パラメータ(α/τ/ε + 任意追加)| WP114 のスカラー宣言 |
| ジッタ系列 | WP115 の `pattern: "table"` — 位相数・配置とも数表直書き |
| 丸ごと差し替え | checkerboard・アキュムレーション・アップスケーラ実験を同機構上に |

**エンジン語彙(版付き additive — いじれないことが安全の根拠)**:

1. ジッタ適用点(scene ラスタのみ・カリング/rpc/ゲームプレイ非ジッタ)
2. 単一提供者の排他
3. FrameUBO の供給 field(jitter_ndc ペア・epoch ペア — 形式凍結・
   読み方は自由)
4. history RT の 2 面フリップのタイミング
5. velocity の元データ保持(前フレーム行列・palette — シェーダは
   コピー改造可・保持機構はレンダラ内部)
6. epoch 増加イベントの定義(resize/set_time/カメラ不連続)

**昇格手順**: ユーザー改造がエンジン側 6 項の変更を要求したら、それは
語彙の不足 — 実装で回避せず **additive な語彙追加 WP** を切る
(A1.1(WP102)・J1b(WP114)・J1c(WP115)の確立済み手順)。
この境界表は adding_features.md にユーザー向けの言葉で転載する
(WP115 の成果物)。

...
## 著作キャンバスの設計(2026-08-20)

**要求 1「ノードエディタで 0 からパスを組めるようにしたい」に対する設計。**

**書く前に既存設計 45 本を調査した**(前回、受理済み契約を読まずに 4 回設計を書いて捨てた)。
結論: **ノードエディタの設計はどこにも無い。しかし書く先はほぼ全部決まっている。**
本設計はその制約の中で UI を決めるものであり、著作形式を発明しない。

### 利用者が述べた使い方(2026-08-20)

> デフォルトのフォワード/ディファードとかは preset で与えて、
> その後ろのフルスクリーンパスとかはよくいじられると思う。
> でも複雑なパス設定も設計できるようにはしたい。

**これは形式の主用法と一致する。**出荷 feature 18 本の `insert` を数えると:
...
**「土台の後ろに足す」が 4 倍の多数派である。**

### したがって中心の操作は「0 から組む」ではない

**主操作は「既存のパスを指して、その後ろに足す」である。**

これは `insert: "after:<pass>"` そのままであり、**GUI の身振りと形式が 1 対 1 で対応する。**
中間形式を発明する必要がない。

「0 から組む」は、その操作を繰り返した結果として到達する状態であって、
別のモードではない。**空のキャンバスから始める UI を作らないこと。**

### 編集の対象は「グラフ」ではなく「自分の fragment」

キャンバスが描くのは**合成後のプラン**(preset + feature + 自分の著作)である。
しかし**編集できるのは自分が著作した部分だけ**である。

| ノードの出自 | 扱い |
|---|---|
| 自分の著作 config / 自分の fragment | **編集できる** |
| preset 由来 | **編集できない。**所有する道を名前付きで示す |
| engine feature 由来 | 同上 |
| canonical anchor / `output_transform` / terminal / present / XR / barrier | **編集できない。削除・切断できるように見せない** |

**preset / feature 由来を所有する正規の操作は決まっている** ——
「engine 標準 fragment を project へコピーし、`features` の参照を書き換える」。
**暗黙シャドーイングは禁止。**UI はこれを「コピーして所有する」として明示的に行うこと。
**黙って編集可能にしないこと。**

### 辺は著作されない

**現在のキャンバスの辺は `execution_plan.dependencies`、すなわちコンパイラが導出した依存である。**
著作側にあるのは `input` / `output` の**リソース名の参照だけ**で、
辺はそこからエンジンが導出する。

**したがってキャンバスで「辺を引く」身振りは、実際には
「このパスの `input` にこのリソース名を足す」である。**
**依存を直接描いていると誤解させないこと。**

宣言順は**安定タイブレークであって意味論ではない**。
明示 `before` / `after` は、データ依存で表せない意図のためだけの契約である。
**順序が導出できない writes-writes は hard error** であり、
解消は明示エッジか中間リソースである。**UI が黙って順序を決めないこと。**

### 自動配線の規則(既決)

**候補がただ 1 つのときだけ自動で繋ぐ。**複数あるなら**推測せず候補名と接続例を示す。**
明示的な接続は常に優先する。

**未指定は「制約なし」という作者の主張として受理する。**
`parallel_safe` のような positive boilerplate を要求しない。

### 判定の三値は既存のフォームと同じ

studio が判定できるのは **field ownership / 名前衝突 / ターゲット名の存在**、
そして **WP323 で usage が加わった**。
**生成順とシェーダー stem の解決は依然として判定できない。**

**「妥当」と「判定していない」を同じ状態にしない。軸の名前を挙げる。**
**「とりあえず動く既定へ戻す」挙動を作らないこと** ——
未知名・version 不一致・provider 未登録は名前付きの compile error である。

### 天井 —— どこまでを著作キャンバスで扱うか

利用者は「複雑なパス設定も設計できるようにしたい」と述べた。

拡張点は **8 段の独立した ladder** として確定している
(preset → ResourcePattern → PassImplementation → region/subgraph →
GraphTransform → RenderStrategy → PhysicalLowering → NativeScope)。

**著作キャンバスが届くのは、著作 JSON で表現できる段までである。**
それより下は C ABI の provider であり、**JSON では書けない。**

**天井を隠さないこと。**キャンバスは、その先が必要になった時点で
**「ここから先は provider が要る」と名前付きで示す**こと。
**8 段を 1 枚のグラフに平坦化して単一 API へ流すのは、明示された非目標に違反する。**

また `region tag` は**置換・診断・eject anchor・grouping のタグ**であって、
barrier / allocation / fusion / alias / 最適化境界ではない。
**「囲むと隔離される箱」として提示しないこと。**

### キャンバスとプランの関係

**view-model は分ける。**著作側の接続は pass↔resource の名前参照、
プラン側の辺は導出された pass→pass 依存で、**辺の意味が違う**。

**しかし widget を分けることは論証されていない。**
編集後・反映前を amber/pending、コンパイル済みを generation N と並べるのは矛盾ではなく、
**利用者が見たい差分そのものである。**

**同期 split view / 同一シーンのレイヤ切替 / 重ね表示を比べてから決めること。**
linked selection と共有レイアウトを受け入れ条件にする。

**`nodes[*].kind` は著作カテゴリ兼診断であって execution endpoint ではない。**
「どこで実行されるか」として見せないこと。
**最終プランに要求されるのは connected ではなく closed** であり、
非連結な部分があるのは正常である。**警告にしないこと。**

### 反映は既存の一本に乗る

WP331/332 で通した経路をそのまま使う ——
engine が document を所有し、edit は ticket を返し、
保存は preflight → 一時ファイル → atomic replace(digest CAS)、
反映は `pelican.render_pipeline` を名指しした `applyRuntimeNow`。

**別経路を作らないこと。**`ReloadGate` を迂回しないこと(`can_edit` が独立 query)。

現在の編集 RPC は **`operations` が v1 で要素 1 個まで**である。
**パスの追加は複数の操作を要する**(fragment への pass 追加 + `features` への参照追加)ので、
**この制限を先に外すか、1 操作で表せる形に落とすかを決めること。**

### ノード位置 —— ワークスペースの 3 層目

**まだ空である。**WP330 はパネル配置とツール内配置の 2 層を実装し、
**ノード位置の永続化を範囲外にした**(`session_node_positions_` は
「never reach disk」のままである)。

著作キャンバスはノードを置く場所が意味を持つので、**この層を埋めることになる。**
保存先と作法は「エディタのワークスペース状態」節が決めている ——
**プロジェクトの中に書かない。プロジェクトを鍵に `AppConfigLocation` 配下。共有は明示的 export のみ。**

### 段階

```
1. 既存パスを指して「後ろに足す」—— insert アンカーの UI と、1 パスの追加
2. 自分の著作したパスの編集(WP321a のフォームをキャンバスから開く)
3. preset / feature 由来を「コピーして所有する」
4. 複数操作のトランザクション(operations の 1 個制限を外す)
5. ノード位置の永続化(ワークスペース 3 層目)
```

**段階 1 が単独で価値を持つこと。**後続が来なくてもコードベースが整合すること。
このリポジトリには誰も使っていない機構が既に複数ある。**増やさないこと。**

### 受け入れ条件は段階 1 の WP で書く

最低限、次を逐語で書くこと:

- **既存のパスを指して fullscreen パスを 1 つ後ろに足すと、
  著作ファイルに `insert: "after:<そのパス>"` が書かれること**
- **アンカーが曖昧・不在のときに名前付きで拒否されること**(黙って `end` に落とさない)
- **preset / feature 由来のノードを指して足せること**
  (アンカーにするのは合法。**編集とは別である**)
- **編集できないノードが、編集できないと分かること**
- **足した結果が再起動なしでプランに現れること**(WP331 の経路に乗る)

...
(最新完了: WP193、2026-07-24。本文と完了レポートは archive 参照。)

## 2. WP 詳細

現在アクティブな WP はない。WP193 で pass / frame graph / logical・physical plan /
route / sample / variant policy・draw-sort provider 選択の immutable runtime generation、
prepare/rollback、base-generation CAS publish、frame lease を実装済みである。
renderer の次候補は RPE10b の GPU registry candidate / fence retire / watcher 接続、
または material fragment-output schema / tile-native rendering scope の独立縦切りである。
...
- **レンダーパイプライン拡張境界(RPE、2026-07-23 v2.1 / HEG v1)**: versioned
  ...
  builtin graph variant policy(WP192)と runtime publication root(WP193)まで完了。
  次は pipeline transaction のGPU resource側(RPE10b)を独立WPとして進める。

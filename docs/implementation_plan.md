...
`projects/example` の 20 パスのうち **16 が `fullscreen`**。その形は:
...
#### 全ての項目が列挙可能な集合からの選択である

| 項目 | 選択肢 | 実測 |
|---|---|---|
| `type` | 列挙 | example は `fullscreen` 16 / `material` 2 / `snapshot_copy` 2 |
| `shader.vertex` | fullscreen なら実質固定 | 20 パス中 16 が `engine://fullscreen` |
| `shader.fragment` | シェーダー資産 | エンジン 18 ファイル + プロジェクト 3 |
| `input` | 既存ターゲット | 22 |
| `output.color` | 既存ターゲット / 新規 | 22 |
...
1. パスを組むフォーム。上表の項目を、**列挙された選択肢から選ぶ**形で提供すること。
   自由入力は名前のみ。
2. **組んだ結果の JSON をその場に表示すること。**利用者がコピーできること。
3. **入力の妥当性をその場で示すこと。**
   存在しないターゲットを参照していないか、名前が既存と衝突しないか。
   **エンジンの検証規則を studio 側に写経しないこと** ——
   `pelican_project` にある既存の検証を使うか、使えないなら
   「ここでは検査していない」と明示すること。**二重の流儀を作らない。**
4. 選択肢の出所を実データにすること。ターゲットは現在のフレームプランから、
   シェーダー資産は列挙から。**ハードコードした一覧を作らないこと。**

#### 範囲外

**プロジェクトへの書き込み。エンジンへの反映。位置やフォーム状態の永続化。**
既存パスの編集(本 WP は新規作成のみ)。

#### 受け入れ条件(§4 規約 10)

- **フォームで組んだ JSON が、`projects/example` の既存パス宣言と同じ形であること。**
  例えば `ssao_pass` と同じ選択を行うと、`main_rendering_config.json` の
  当該エントリと**意味的に一致する JSON** が出ること —— これが本 WP の中心的な対照である
- **存在しないターゲットを参照した状態で、妥当と表示されないこと。**
  名前が既存パスと衝突する場合も同様
- 選択肢が実データ由来であること。**フレームプランのターゲットを 1 つ増やすと、
  選択肢も増えること**(ハードコードなら増えない)

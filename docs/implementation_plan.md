...
1. **ターゲットを選ぶと、その周辺の部分木を描くこと。**
   既定の深さは生産者側・消費者側とも 1 段。深さは利用者が変えられること。
   **推移的に全部辿る既定にしないこと** —— それは全体を見るのと同じになる。
2. **物理層の判断を、その場に重ねること。**
...
5. **ノードをドラッグで動かせること。** `ItemIsMovable`。
...
- **flat / preview / xr と `PELICAN_RUNTIME_SHADER_COMPILER` の ON / OFF で成立すること**
- `ctest` 全数が緑(`-j4`)、`uv run tools/doclink.py check` が通ること
...
- `lifetime{first_use, last_use}` は wire にある(エンジンが算出済み)
- **WP305 が既に studio 側でパースしている**
- 再利用(`alias_groups`)は WP307 が既に表示している
...
- `lit_color` の区間が 3..23 であること。実値と照合すること
- **`alias_groups` が空の構成(`conservative_debug` profile)で、
  再利用の表示が消え、区間の表示は残ること** —— これが本 WP の対照である
- **`QT_QPA_PLATFORM=offscreen` で緑であること**
- `ctest` 全数が緑(`-j4`)、`uv run tools/doclink.py check` が通ること

依存: なし(WP318 と独立。行から部分木へ飛ぶ機能だけが WP318 に依存)。見積: 小〜中。

...
**① 層別レイアウトにすること(本 WP の主眼)。**

- ノードを**依存の深さ**で層に割り当てること。実行順の一列に並べないこと。
  同じ層に入るのは、互いに依存の無いノードである
- **層内の並びは辺の交差が減るように決めること。**
  30 ノードの主鎖にダイヤモンドが 2 つ、スキップ接続が 3 本あるので、
  交差を減らすかどうかで読めるかが決まる
- 決定的であること。**乱数による初期化や再試行を使わないこと**
...
**当面は studio 側に持つこと**(プロジェクトファイルに書かない)。
理由: 現在キャンバスは閲覧専用であり、位置は文書の内容ではなく表示状態である。
編集機能が入った時点で「位置は文書の一部か」を決め直せるよう、
**保存形式は `(graph, name)` をキーにした素直な対応表にしておくこと。**

...
**30 ノードを固定倍率・固定配置・直交折れ線で見ることになるので、密集して追えない。**

#### 実装範囲

1. **辺を曲線にすること。** 出力側から入力側へのベジエ。
   直交レーン配線をやめる。辺が交差しても追えるようにすること。
2. **ホイールでズームすること。** 変換アンカーは既に `AnchorUnderMouse` なので、
   `wheelEvent` で `scale()` を呼ぶだけで、カーソル位置を中心に拡縮する。
   ズーム範囲の上下限を決め、範囲外の入力を黙って無視しないこと。
3. **ノードをドラッグで動かせること。** `ItemIsMovable`。
   動かした位置は `(graph, name)` をキーに保持すること。
4. **ポートを描くこと。** ノードの縁に入出力の接続点を置き、
   **辺はノード中心ではなくポートに接続すること。**
   1 ノードに辺が集中したときの可読性はここで決まる。
...
- **利用者の移動が無い状態で、入力配列の順序を入れ替えても座標が一致すること。**
  WP306 の決定性条件が生きていること —— これが本 WP の中心的な対照である
...
自動レイアウトアルゴリズムの作り替え —— **本 WP は見え方と操作であって、配置の質ではない。**

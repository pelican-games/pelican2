...
2. **その一部を無視できること。** 全部か無かではなく、**部分ごとに**受け取るかを選べる。

この 2 つから、**共有層と局所層の二層**が要る:

- **共有層**(プロジェクトと一緒に渡る): ノード位置、畳み設定、名前付きのビュー構成など。
  **人が作業して作った並び**であり、失うとやり直しになるもの
- **局所層**(利用者・機械ごと): ウィンドウ形状、ドック配置、スクロール、選択、最後に開いたもの。
  **画面サイズや好みに依存**し、他人に押し付けるべきでないもの

そして**局所が共有を上書きし、共有層は部分ごとに拒否できる**こと。
前例: Unity の `ProjectSettings/` と `UserSettings/`、VS Code の `.vscode/settings.json` と利用者設定。
どちらも共有側が小さく読める形で、局所側が勝つ。

### 設計で答えるべきこと

- 二層の境界をどこに引くか。**「失ったらやり直しになるか」**が判定基準になりうる
- 部分ごとの無視をどう表現するか(受け取る側の宣言か、送る側の分類か)
- 共有層を git 管理するか。**diff に出ることを許容するか**

...
### WP318: ノードキャンバスを実際に読める操作感にする

**目的**: WP306 のキャンバスを起動して確認した結果、**利用者の評価は「かなり見づらい」**。
Unity / Blender のノードエディタと同じ操作感にする。

#### 現状(起動して確認、2026-08-17)

**根本原因は配置である。** `frameplangraphics.cpp:912-916`:

```cpp
qreal cursor_x = 0.0;
for (VisibleEntity *entity : ordered_entities) {
    entity->geometry = QRectF{cursor_x, 0.0, width, height};   // y は常に 0
    cursor_x += width + HorizontalGap;
}
```

`ordered_entities` は `(order, key)` で整列済み、つまり**実行順**。
**全ノードが一直線に並び、`y` は全部 `0.0`。**
30 ノード × 240px で**横 7000px 超の一本線**になり、辺だけが上下のレーンを通る。

**キャンバスはトポロジカルソートを箱に描き直しているだけである。**
利用者が「ノードとしてみる意味があまりない」と言った状態そのものが画面に出ている。
WP306 が「消えた 5 辺を見せる」ことに成功していても、**ノードの並びが線のままなら構造は見えない。**

補助的に、次も欠けている:

| 要素 | 状態 |
|---|---|
| パン | 効く。`ScrollHandDrag`(`src/devstudio/view/frameplanwidget.cpp:229`) |
| **ズーム** | **効かない。** `AnchorUnderMouse` は設定済み(`:230`)だが、
  `wheelEvent` が `src/devstudio/view/` のどこにも無く `scale()` を呼ぶ箇所が無い |
| **ノード移動** | **できない。** `ItemIsSelectable` のみ(`frameplangraphics.cpp:935`)で
  `ItemIsMovable` が無い |
| **辺** | **直交折れ線。** `lineTo` を 5 回、レーン配線(`frameplangraphics.cpp:974-979`) |

#### 実装範囲

**① 層別レイアウトにすること(本 WP の主眼)。次を仕様として確定する。**

- **層(rank)の定義**: 依存辺に沿った最長路長。入次数 0 のノードが rank 0。
  実行順(`order`)を rank に使わないこと
- **層内順序(lane)の決め方**: 中央値法(median heuristic)を
  **前方 → 後方の 2 掃引で計 4 回、固定回数**行う。反復回数を入力に依存させないこと
- **初期順序**: rank 内で `(order, key)` の昇順。`ordered_entities` の既存整列と同じ規則
- **タイブレーク**: 中央値が等しい場合は初期順序を保つ(安定ソート)。
  **すべての比較に全順序を与えること** —— 未定義の同値を残さない
- **乱数・再試行・時刻・ハンドル値を一切使わないこと**
- **交差数の定義**: 隣接する rank の対について、辺の端点 lane の順序が
  逆転している辺対の個数。この定義を実装とテストで共有すること
- **畳みとの関係**: グループは**単一のノードとして 1 つの rank に置く**。
  構成員が rank を跨いでいた場合、グループの rank は構成員の最小 rank とする。
  展開時は構成員それぞれの rank に戻ること

**② 辺を曲線にすること。** 出力側から入力側へのベジエ。直交レーン配線をやめる。

**③ ホイールでズームすること。** 変換アンカーは既に `AnchorUnderMouse` なので、
`wheelEvent` で `scale()` を呼べばカーソル位置中心で拡縮する。
上下限を決め、範囲外の入力を黙って無視しないこと。

**④ ノードをドラッグで動かせること。** `ItemIsMovable`。
**位置はセッション内(メモリ上)のみ。ディスクに保存しないこと。**
理由は下記「位置の永続化を本 WP で決めない」を読むこと。

**⑤ ポートを描くこと。** ノードの縁に入出力の接続点を置き、
**辺はノード中心ではなくポートに接続すること。**
**ポートの単位はリソースとする** —— `reads` / `writes` から資源別の端子を構成する。
仕様レビューで `projects/example` の資源依存 37/37 が現在の wire だけで構成可能と検算済み。
**論理ポート名や多重ポートの同一性は wire に無いので、それらを要求しないこと。**

#### WP306 の決定性条件との関係(重要)

WP306 は受け入れ条件に
**「入力配列の順序を入れ替えても座標・束ね順・色が一致すること」**を持つ。
ノードを動かせるようにすると座標が利用者の状態になるので、条件を次のように**分けること**:

- **自動レイアウトは従来どおり決定的であること。**
  利用者の移動が無い状態では、入力順を変えても同じ座標になる。**この条件を弱めないこと。**
- 利用者が動かした位置は**別の層**として保持し、自動レイアウトに上書きを重ねる形にすること。

#### 位置の永続化を本 WP で決めない(仕様レビューの結果)

当初は「studio 側に `(graph, name)` をキーで保持」と書いていた。**そのキーは成立しない。**

- **`FramePlanNodeKey` 自身が「persistent/stable id ではない」と明記している**
  (`src/devstudio/model/frameplanmodel.hpp:15`)
- **`sprite_demo` と `vrm_xr_demo` はどちらも graph `main` に
  `gbuffer_pass` / `ssao_clear` / `lighting_pass` を持つ。**
  プロジェクトを跨いで衝突する
- `example` と `animgraph_demo` も preset 経由で `main_render` を共有し、ノード名が重なる
- XR では compute ノードに `#xr` が付き、variant を跨いで同一性が保てない
- **`FramePlanWidget` にプロジェクトルートが渡されていない。**
  プロジェクト識別子は `MainWindow`(`view/mainwindow.cpp:183`, `:320`)と
  private な viewport 状態にしかない

**ウィジェットは自分がどのプロジェクトを表示しているか知らない。**
したがって位置の保存先は本 WP の中では決められない。

**本 WP はセッション内(メモリ上)の移動までとする。**
永続化は「エディタのワークスペース状態」の設計に従うこと ——
パネルのドッキング、ビュー内部の状態(位置・畳み・展開・列幅)は
**同じ系で解決すべき**であり、個別に決めると仕組みが増え続ける。
現状はドック配置が `LayoutPresetManager`(AppConfig、名前付き手動保存、自動復元なし)にあるだけで、
プロジェクト単位のエディタ状態は 1 つも存在しない。

#### 受け入れ条件(§4 規約 10)

- **`projects/example` の 30 ノードが 2 層以上に配置されること。**
  現状は全ノードが `y == 0` の一列である。**これが本 WP の中心的な対照である** ——
  層の数と、各層に属するノード名を実値で検査すること
- **互いに依存の無いノードが同じ層に入ること。**
  例: SSAO の分岐(`ssao` / `ssao_blur`)と、それを迂回する `gbuffer_pass → lighting_pass` の直行辺が、
  ダイヤモンドとして見える配置になること
- **辺の交差数が、実行順に一列に並べた場合より少ないこと。**
  両方の配置で交差数を数えて比較すること
- **利用者の移動が無い状態で、入力配列の順序を入れ替えても座標が一致すること。**
  WP306 の決定性条件が生きていること
- ノードを動かした後、**同一セッション内で**その位置が保たれること。
  **feature を切って別のノード集合になったとき、消えたノードの位置が孤児として残らないこと**
  (scene は毎回 clear されるので、scene を見るだけでは孤児の有無は分からない。
  保持している対応表そのものを検査すること)
- ズームの上下限を超える入力が、**黙って無視されず**に上下限で止まること
- 辺がポートに接続していること。ノード中心から出ていないこと
- **`QT_QPA_PLATFORM=offscreen` で緑であること。**
  グリフ計測に依存する判定を入れないこと —— レイアウトが確保した領域を見ること。
  **「未設定側でも緑」を条件にするなら、別の CTest 登録を足すこと** ——
  `QT_OFFSCREEN` は discovery 全体に環境変数を付ける(`test/CMakeLists.txt:108`)ので、
  現行の登録一つでは未設定側が証明できない
- **flat / preview / xr と `PELICAN_RUNTIME_SHADER_COMPILER` の ON / OFF で
  rank・座標・ポート・交差を検査すること。** WP316 が要求した variant 網羅を引き継ぐこと
- **依存 0 件の正当なグラフと、非連結成分を含むグラフでノードが重ならないこと**
  (`src/project/executionplan.hpp:117` は非連結成分を正式に許可している)
- `ctest` 全数が緑(`-j4`)、§0 の反転対象のうち触れたフラグを反転して緑、
  `uv run tools/doclink.py check` が通ること

#### 範囲外

編集(ノードの追加・削除・辺の張り替え)。位置のプロジェクトファイルへの保存。

依存: なし。見積: 中〜大。

## エディタのワークスペース状態 — 設計の前提(2026-08-17)

WP318 で「ノード位置をどこに保存するか」を単独で決めようとして破綻した。
利用者の指摘:

> エディタは json の記述を GUI でサポートするもので、それを様々な形でエディタに実装した上で、
> **それのドッキングを管理するパラメータと、その内部での GUI 配置を解決するべき**だから、
> とても関連があると思う

**パネルのドッキングと、ビュー内部の状態(ノード位置・畳み・展開・列幅・選択)は一つの系である。**
個別に決めると仕組みが増え続ける。現に今、次がバラバラである:

| 状態 | 現状 |
|---|---|
| ウィンドウ・ドック配置 | `LayoutPresetManager`(AppConfig、**名前付き手動保存、自動復元なし**) |
| ノード位置 | 無い(WP318 でセッション内のみ) |
| 畳みのしきい値 | **保存されない** |
| タブ選択・ツリー展開・スクロール | **保存されない** |
| プロジェクト単位のエディタ状態 | **1 つも存在しない**(`.pelican/` は派生キャッシュ専用で gitignore) |

### 利用者が決めた要求(2026-08-17、対話で確定)

1. **ワークスペースの情報をプロジェクトと一緒に渡せること。**
   他の人が開いたときに、こちらの並べ方が伝わる。
2. **同じプロジェクトに対して、ワークスペースを複数持てること。**
   理由は利用者の言葉で:
   > デザイナー、エンジニアそれぞれが使いやすいエディタを使ってほしいので複数持ちたい
3. **ワークスペースはプロジェクトの内部データではない。**
   **エンジンはこのファイルを一度も読まない。**
   したがって形式のサブセット原則(web で開ける ⊆ pelican で開ける)は関与しない ——
   プロジェクト形式の拡張ではなく、「エンジンが無視するファイルを置く」話である。

### 確定した形(2026-08-17、対話で決着)

**2 層に分ける。どちらも独立に名前付きで、独立に渡せる。**

```
層1 パネルレイアウト（人に付く・プロジェクト非依存・名前付き複数）
     例: "designer" / "engineer" / "debug"
     中身: どのパネルがあるか、相対配置

層2 ツール内レイアウト（プロジェクトをキーに持つ・名前付き複数）
     例: main_render の「流れで読む」/「バリアで読む」
     中身: ノード位置、畳み、フィルタ

機械ローカル（どちらの層にも入らない・構造的規則）
     ウィンドウ実座標、モニタ位置、最後に開いたもの、スクロール、選択
```

**層を分ける理由**(利用者の言葉):

> ノード配置のようなものは整理結果をパネルレイアウトとは別に共有したいことはありそう

**整理結果は労働の成果である。**30 ノードを読める形に並べたなら、
相手がこちらのパネル配置を使うかどうかとは無関係に渡したい。
逆も言える。**2 層が独立に選べることで、当初の「部分ごとに無視できる」が
層という自然な粒度で満たされる。**フィールドごとのスイッチは不要。

### 既定はプロジェクトの外。「渡せる」は明示操作(重要)

一度は「層 2 はプロジェクトと一緒に渡る」と書いた。**却下する。**

**自動保存と git 管理は両立しない。**
ノード配置は自動保存でないと意味がない(手動保存を忘れて失うのが最も痛い)。
プロジェクト内に置いて自動保存すると:

- **作業ツリーが永久に dirty になる。**
  §0 の完了条件は「`git diff --check` クリーン」であり、それが常に汚れる
- **マージ衝突。** 二人が同じグラフを並べたら毎回衝突し、
  しかも誰も真剣に読まないファイルで衝突する。正しい解決が誰にも分からない
- **diff ノイズ。** 実質的な変更の review が座標の変動に埋もれる

そして利用者自身の宣言
「**基本的にプロジェクトの内部データではない**」に反する。
置けばいつか誰かが依存し始める。

**要求は「渡せる」であって「常に一緒に付いてくる」ではない。**

```
既定: プロジェクトの外（人に紐づき、プロジェクトをキーに持つ）
      → 自動保存してよい。git を汚さない。衝突しない

共有: 明示的に書き出す / 読み込む
      → 「この配置を渡す」という意思のある操作

プロジェクトに入れたい場合も、明示的に書き出せばよい。
既定ではなく選択になる。
```

これで層 1 と層 2 が**対称**になる。どちらも人に紐づき、名前付きで、明示的に渡す。
違いは層 2 がプロジェクトをキーに持つことだけ。

**代償**: clone した人に配置は付いてこない。
永久に dirty な作業ツリーと引き換えにする価値は無いと判断した。

### 既存の置き場は層 1 について正しかった

層 1 は人に付くので、`LayoutPresetManager` が `AppConfigLocation` に置いているのは**正解**である。
「ノード位置と揃えるべきか」を検討したが、**揃えるべきではない** —— 層が違う。

既存機構に足りないのは:

- **自動保存**(いまは名前付き手動のみで、終了すると配置が消える)
- **ファイルとして渡せること**
...
- 層 2 の置き場と、プロジェクトの同定方法。
  プロジェクトが移動・複製されたときにどうなるか
- **相対配置をどう表現するか。** `saveState()` のバイト列は機械依存を含みうるので、
  そのまま共有用の形式に使えない可能性がある。実際に何が入っているか確かめること
- **書き出し / 読み込みの粒度。** 層まるごとか、その中の名前付き一つか
- 自動保存の頻度と、保存中に落ちた場合(原子的置換が必要か)
- **ビューが増えたときに壊れない形か。** 新しいパネルを足すたびに形式を変えないこと
- `LayoutPresetManager` をどう吸収するか。捨てずに層 1 の一機能として残すこと
- 自動保存か明示保存か。現状はドック配置ですら明示のみで、**終了すると消える**
- **ビューが増えたときに壊れない形か。** 新しいパネルを足すたびに保存形式を変えないこと
- ノードの同一性 —— `(graph, name)` はプロジェクトを跨いで衝突する
  (`sprite_demo` と `vrm_xr_demo` がどちらも graph `main` に `gbuffer_pass` を持つ)。
  **共有層のキーは何にするか**

### 設計手順(前回の失敗を繰り返さないこと)

編集側の設計は 3 回不合格になった。原因は
**私が誤った前提を「再設計禁止」として渡し、Claude 側のレビュアが誰も疑えなかった**こと。
本設計では:

- 前提は**検証対象として**渡すこと
- **最初から codex にも見せること。** Claude のレンズだけを何枚重ねても盲点は共有される
- 評価軸に**「利用者の要求を表で照合する」**を入れること ——
  機構の粗探しに終始して要求充足を誰も問わない、という失敗を繰り返さない

...
当初は「一つの塊を部分ごとに受け取るか選ぶ」形を想定し、
共有層 / 局所層の二層と部分ごとの拒否スイッチを置こうとしていた。**それは不要である。**

ワークスペースが**名前付きの複数文書**なら、拒否は**文書の単位**で表現できる:

```
想定していた形: 一つの塊があり、部分ごとに受け取るかを設定する
確定した形    : 複数の文書があり、どれを開くかを選ぶ（設定が要らない）
```

「designer 用を開く」「engineer 用を開く」「自分のを作る」で足りる。

**既存の `LayoutPresetManager` は既に「名前付きの並べ方を複数持つ」機構**であり、
この設計はその一般化にあたる。捨てるのではなく、ワークスペース文書の一機能として吸収する。

### ただし機械依存の値は文書に入れないこと(構造的な規則)

designer のワークスペースを engineer が開いたとき、
**designer のモニタのピクセル座標が来ては困る。**
これは利用者が設定で選ぶ話ではなく、**入れてはいけない**という規則である。

```
ワークスペース文書（複数・名前付き・プロジェクトと一緒に渡る）
  ├─ どのパネルがあり、どう並ぶか（相対配置・比率・分割位置）
  ├─ ノード位置、畳み、フィルタ
  └─ 役割向けの構成そのもの
...
判定規則: **機械に依存するものは文書に入れない。**

**これは現状と衝突する。** `LayoutPresetManager` は `saveGeometry()` で
実ピクセルのウィンドウ形状を保存している(`view/mainwindow.cpp:745`)。
共有する文書には**絶対座標ではなく相対配置**を持たせる必要がある。

### 設計で答えるべきこと

- ワークスペース文書をどこに置くか。プロジェクトの中か隣か。
  **既定で一緒に渡り、かつ移動・複製・複数持てる**こと
- `dist-config` やマニフェストが「知らないファイル」を許すか。
  厳密な列挙を要求する仕組みがあると引っかかる
- 相対配置をどう表現するか(`saveState()` のバイト列は機械依存を含みうる)
- git 管理するか。**diff に出ることを許容するか**

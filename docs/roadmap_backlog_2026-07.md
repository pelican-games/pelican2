...
21. **hybrid deferred + forward 基盤** [完了] — versioned preset、semantic
    ...
    RPE1(WP180、完了)→ RPE2 typed plan(WP181、完了) → DrawQueueBuilder/provider → transparent sort →
...
| 順 | 候補 | 理由／前提 |
|---|---|---|
| 次 WP 候補 | **RPE3: `DrawQueueBuilder`** | 現行 draw inventory と queue materialization を分離し、`state_batched_v1` で indirect bytes / draw ranges / 二回実行一致を固定する。着手前に WP 登録 |
...
| 6 | **RPE4〜RPE6** | provider registry、transparent sort、hybrid screen input。WP180/WP181 の境界と RPE3 fixture を確認して一段ずつ登録 |

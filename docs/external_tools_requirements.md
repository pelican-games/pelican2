...
この文書は**自己完結**している。ツール側のリポジトリにコピーして使ってよい(その場合この文書が正本、こちらは参照コピーとする旨を明記すること)。
...
**R5: 頂点キャッシュ(布・流体・ソフトボディ)は Alembic を第一候補とする。** 必要になるまで実装しない。実装時はこの文書を更新して両プロジェクトで合意してから。
...
**R8: ライブ連携する場合は JSON-RPC 2.0 over TCP(localhost)または stdio。**

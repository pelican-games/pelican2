...
| 層 | 内容 | 置き場所 |
|----|------|---------|
| 定義 | perspective / orthographic のパラメータ(glTF と 1:1) | camera コンポーネント(scene v1)/ glb カメラノード |
| コントローラ | orbit / follow / fly / fixed。対象・距離・角度・減衰 | コンポーネント params(`controller` キー)。実装はゲームシステム様式(エンジン組み込みのビルトインシステム) |
| 演出 | カット切替・ブレンド(補間曲線)・(将来)シェイク | v1 はカット切替のみ。ブレンドは v2。transform_seq v2 のカメラトラックが「ベイク済み演出」の受け皿 |

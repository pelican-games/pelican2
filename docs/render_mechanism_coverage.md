...
| 機構 | 現在の契約 | 根拠 |
|---|---|---|
| render target | fixed/output-relative extent、format/class/role、usage、history、format candidates、fixed/full mip、array layers、2D/runtime cube resource shape、fullscreen/compute/material/raster attachment subresource view | [rendertargetjsonparser.cpp](../src/core/renderingpass/rendertargetjsonparser.cpp) |
...
| **G10b（部分解消、WP235〜236）** | raster attachmentの任意1 mip/連続layer出力とruntime cube image、face attachment、sampled cube viewを実装。runtime 3D image/viewは未公開 | froxel、runtime 3D LUT |

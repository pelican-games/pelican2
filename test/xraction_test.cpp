...
TEST_CASE("OpenXR actions are created and Touch bindings suggested before the one attach",
    ...
    const auto actions = inputActions();
    {
        ...
        REQUIRE(fake.suggestions.size() == 4);
        ...
        CHECK(fake.actions.size() == 3); // pose is intentionally reserved for XR3b.
        ...
        const auto inactive = runtime.sync({"gameplay"}, false);
        CHECK(fake.calls.size() == calls_before_focus_loss);
        CHECK_FALSE(inactive.get("select").held);
        CHECK(inactive.get("throttle").axis1 == 0.0F);
        CHECK(inactive.get("move").axis2.x == 0.0F);

        const auto frame = runtime.sync({"gameplay"}, true);
        REQUIRE(fake.synced_sets.size() == 1);
        CHECK(fake.sets.at(fake.synced_sets.front()) == "gameplay");
        CHECK(frame.get("select").pressed);
        CHECK(frame.get("select").held);
        CHECK_FALSE(frame.get("select").released);
        CHECK(frame.get("throttle").axis1 == 0.75F);
        CHECK(frame.get("move").axis2.x == 0.25F);
